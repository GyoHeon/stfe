        ...
        if (Math.random() * 100 < addExtraEndSoundDegree) {
          converted =
            extraEndSoundCandidates[
              Math.floor(Math.random() * extraEndSoundCandidates.length)
            ];
          arr.push(converted);
        }
      } else {
...
const convertToAsItSounds = (charArr, asItSoundsDegree, onlyWansungChars) => {
  ...
  for (let i = 1; i < charArr.length; i++) {
    ...
    if (Hangul.isHangul(c1) && Hangul.isHangul(c2)) {
      ...
      if (endPlusHConversionTable.hasOwnProperty(c1end) && c2beg === "ㅎ") {
        c1cs.pop();
...
const doubleConsonantAsEndSound = (
  ...
  for (let i = 1; i < charArr.length; i++) {
    ...
    if (Hangul.isHangul(c1) && Hangul.isHangul(c2)) {
      ...
      const c2beg = c2cs[0];
      if (c1end === "x") {
        const endAsDoubleConsonant = begToEndAsDoubleConsonantTable[c2beg];
        if (endAsDoubleConsonant !== null) {
          const newC1 = Hangul.a([c1cs[0], c1cs[1], endAsDoubleConsonant]);
          if (onlyWansungChars) {
